...
public class PatientDAO extends DAOBase {
    ...
    public static final String Patient_FName = "PrenomPatient";

    public static final String Patient_LName = "NomPatient";
    ...
    public List<Patient > getAll() {
        ...
        if(cursor!=null) {
            while(cursor.moveToNext()) {
                int id = cursor.getInt( cursor.getColumnIndex("IdPatient"));
                ...
                Patient p = new Patient(id,lName,fName,DateNaissance,SexePatient,PseudoPatient,MotdepassePatient,EtatPatient,dateintervention,l,m,i);
                patients.add(p);
    ...
    public int CheckPossiblity(String pseudoname){
        Cursor cursor = getReadableDatabase().rawQuery("select * from Patient where PseudoPatient = '"+pseudoname+"'" ,null);
            if(cursor!=null && cursor.moveToFirst()) {
                int index = cursor.getColumnIndex("EtatPatient");
    ...
    public Patient getPatient(String pseudo){


        Cursor cursor = getReadableDatabase().rawQuery("select * from Patient p,Lit l,Service s,Ail a,Etage e,Medecin m,TypeIntervention t,Batiment b,Hopital h,Centre c,Ville v " +
                "where p.IdLit=l.IdLit and p.IdMedecin=m.IdMedecin and l.IdService=s.IdService and p.IdIntervention= t.IdIntervention and l.IdAil=a.IdAil and l.IdEtage=e.IdEtage" +
                " and s.IdBatiment=b.IdBatiment and h.IdHopital=b.IdHopital and c.IdCentre=h.IdCentre and v.IdVille=c.IdVille and p.PseudoPatient = '"+pseudo+"'",null);
